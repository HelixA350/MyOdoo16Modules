...
class Xl_work:
    ...
    def __module_tractor(self, sheet, column_modules: str, column_tractors: str) -> dict:
        """Создает словарь с названиями программы ПЭ и числом активных тракторов в ней

        Args:
            sheet (_type_): _description_
            column_modules (str): _description_
            column_tractors (str): _description_
    ...
    def __module_tractor(self, sheet, column_modules: str, column_tractors: str) -> dict:
        ...
        Args:
            sheet (_type_): лист Excel
            column_modules (str): Колонка где написаны названия модулей
            column_tractors (str): Колонка где написан серийные номера тракторов
    ...
    def __text_to_color(self, text)->str:
        """Переводит любой текст в цвет в формате HEX с помощью хэширования, 
        используется для автоматической окраски записей о разных программах ПЭ разным цветом

        Args:
            text (_type_): исходный текст (название программы)
    ...
    def department_stat(self):
        """ Добавляет на каждую страницу бюро небольшую сводку сверху
        """
